...
| MCP creative-tool handoff | Beta | Creators, editors, and generative-video teams | Converts timestamped evidence into a portable edit brief, timeline decisions, and scene prompts for Universal MCP, Canva, HeyGen, or prompt-to-video workflows. | Open a completed video report, choose a destination under **MCP integration**, then copy the tool prompt or download the MCP JSON package. |
