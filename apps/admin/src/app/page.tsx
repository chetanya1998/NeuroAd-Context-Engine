...
function Section({ eyebrow, title, children, guide = "Read the metrics from left to right; hover this icon for the purpose of this section." }: { eyebrow: string; title: string; children: React.ReactNode; guide?: string }) {
  return <section className="dashboard-section"><div className="section-heading"><p className="eyebrow">{eyebrow}</p><h2>{title}</h2><span className="section-help" title={guide} aria-label={guide}><HelpCircle size={16} /></span></div>{children}</section>;
}
...
function QualityLab({ data, user, reload, setNotice, setError }: { data: Json; user: Json; reload: () => Promise<void>; setNotice: (value: string) => void; setError: (value: string) => void }) {
  ...
  return <div className="grid two"><section className="panel"><h3>Grounded feedback queue</h3><p>Record a report error with the evidence needed to correct it. Only reviewed examples can enter the quality set.</p><label className="inline-form">Issue type<select value={issueType} onChange={(event) => setIssueType(event.target.value)}><option value="incorrect_score">Incorrect score</option><option value="unsupported_claim">Unsupported claim</option><option value="missing_evidence">Missing evidence</option><option value="misleading_copy">Misleading copy</option><option value="better_example">Better example</option></select></label><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Describe the expected report output and the evidence to use." /><button disabled={note.trim().length < 8 || !["platform_admin", "ml_operator", "reviewer"].includes(user.role)} onClick={addFeedback}>Approve feedback example</button></section><section className="panel"><h3>GPT-OSS improvement readiness</h3><strong className="quality-count">{data.training_ready ?? 0} approved examples</strong><p>This prepares a versioned training and evaluation set. It does not alter the live model or execute arbitrary training code.</p><button className="secondary-action" disabled={!data.training_ready} onClick={prepareSet}>Prepare quality training set</button><div className="list-row"><span>Mode</span><b>{data.mode?.replaceAll("_", " ") ?? "evaluation"}</b></div></section></div>;
}
