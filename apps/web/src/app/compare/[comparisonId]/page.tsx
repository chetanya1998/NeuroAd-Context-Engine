    ...
    <AppShell>
      <main className="mx-auto max-w-7xl px-5 py-10 lg:px-10">
        ...
        <section className="mt-8 grid gap-4 md:grid-cols-3">
          <DecisionCard title="Highest score in this upload set" value={winner ? displayTitle(winner.title) : "Awaiting videos"} detail={winner ? `${scoreLabel(winner.score ?? 0)} result: ${winner.score}/100. This is a relative ranking, not a campaign approval.` : "At least two videos must complete."} icon={<Trophy className="h-5 w-5" />} score={winner?.score} />
          <DecisionCard title="Why it ranked first" value={winner && runnerUp ? `${scoreGap} ${scoreGap === 1 ? "point" : "points"} ahead` : "—"} detail={winner && runnerUp ? `${displayTitle(winner.title)} scored ${winner.score}/100. The next video scored ${runnerUp.score}/100.` : "A comparison needs at least two completed videos."} icon={<GitCompareArrows className="h-5 w-5" />} score={winner && runnerUp ? scoreGap : undefined} />
